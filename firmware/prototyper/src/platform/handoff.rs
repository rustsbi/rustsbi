...
pub(super) fn prepare_device_tree(
    ...
    device_tree.prepare_next_stage(
        ...
        hidden_node_paths,
        // The placement policy keeps the tree clear of the image even when the
        // platform already reserves it and the reservation argument is `None`.
        Some(memory.firmware_image_range()),
    )
}
