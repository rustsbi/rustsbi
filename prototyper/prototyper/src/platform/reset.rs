...
pub struct P1PmicResetWrap {
    /// I2C controller MMIO base address.
    i2c_base: usize,
...
const ICR: usize = 0x00; // Control register
const ISR: usize = 0x04; // Status register
const IDBR: usize = 0x0c; // Data buffer register
const IRCR: usize = 0x18; // Reset cycle counter
const IBMR: usize = 0x1c; // Bus monitor register
...
/// # Safety
///
/// `i2c_base` must be a valid MMIO address for the I2C controller.
unsafe fn i2c_write_reg(i2c_base: usize, addr_7bit: u8, reg: u8, val: u8) {
    ...
    write32(i2c_base + ICR, CR_GCD | CR_SCLE | CR_MSDE);

    // Wait for the bus to be idle (unit and bus not busy).
    if !spin_until(|| read32(i2c_base + ISR) & (SR_UB | SR_IBB) == 0) {
        return;
    }
    write32(i2c_base + ISR, read32(i2c_base + ISR));

    // START + address byte (write direction): load IDBR, assert START|TB.
    write32(i2c_base + IDBR, ((addr_7bit as u32) & 0x7f) << 1);
    let mut icr = read32(i2c_base + ICR);
    icr &= !CR_STOP;
    icr |= CR_START | CR_TB;
    write32(i2c_base + ICR, icr);

    // Wait for the address byte to be transmitted (TX buffer empty).
    if !spin_until(|| read32(i2c_base + ISR) & SR_ITE != 0) {
        return;
    }
    write32(i2c_base + ISR, read32(i2c_base + ISR));

    // Register address byte (not the last byte: no STOP).
    write32(i2c_base + IDBR, reg as u32);
    icr = read32(i2c_base + ICR);
    icr &= !(CR_TB | CR_ACKNAK | CR_STOP | CR_START);
    icr |= CR_TB;
    write32(i2c_base + ICR, icr);
    ...
    write32(i2c_base + IDBR, val as u32);
    icr = read32(i2c_base + ICR);
    icr &= !(CR_TB | CR_ACKNAK | CR_STOP | CR_START);
    icr |= CR_TB | CR_STOP;
    write32(i2c_base + ICR, icr);
...
mod tests {
    ...
    fn test_register_offsets_match_i2c_k1() {
        ...
        assert_eq!(ICR, 0x00); // Control register
        assert_eq!(ISR, 0x04); // Status register
        assert_eq!(IDBR, 0x0c); // Data buffer register
        assert_eq!(IRCR, 0x18); // Reset cycle counter
        assert_eq!(IBMR, 0x1c); // Bus monitor register
    }
