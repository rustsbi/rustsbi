...
use crate::trap::probe::{ExpectedResult, probe_csr};
use crate::trap::stack;
...
pub(crate) fn prepare_hypervisor_metadata() -> Result<(), TrapInstallError> {
    ...
    if probe_hypervisor_metadata().map_err(|_| TrapInstallError::FeatureProbe)?
        && !crate::trap::probe::enable_hypervisor_metadata(index)
    {
...
pub(crate) fn hypervisor_available(index: usize) -> bool {
    crate::trap::probe::hypervisor_metadata_available(index)
}
...
pub(crate) fn park_current_hart() -> ! {
    ...
    let Some(runtime_stack_top) = crate::entry::hart_stack_top(index) else {
        crate::trap::abort();
    };
    ...
    unsafe { crate::entry::enter_warm_loop(hart_id, index, runtime_stack_top) }
}
