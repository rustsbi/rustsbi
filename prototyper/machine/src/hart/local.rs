...
/// Resolves an arriving physical hart ID before a Rust stack is available.
///
/// # Safety
///
/// The raw entry path calls this only after final runtime publication made the
/// immutable hart table visible to secondary harts.
#[unsafe(naked)]
...
impl LocalInterrupts {
    fn disable() -> Self {
        Self {
            previous: super::instructions::mask_all_interrupts(),
        }
    }

    fn restore(&self) {
        super::instructions::restore_all_interrupts(self.previous)
    }
}
